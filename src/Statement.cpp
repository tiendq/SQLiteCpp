...
bool Statement::isColumnNull(string const &apName) const
{
  checkRow();
  const int index = getColumnIndex(apName);
...
string Statement::getColumnName(const int aIndex) const {
  checkIndex(aIndex);
...
// Return the named assigned to the specified result column (potentially aliased)
string Statement::getColumnOriginName(const int aIndex) const
{
    checkIndex(aIndex);
...
int Statement::getColumnIndex(string const &apName) const {
...
string Statement::getErrorMsg() const noexcept {
